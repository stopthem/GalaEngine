...
void RenderHealthBarsSystem::Update(const std::unique_ptr<Registry>& registry, SDL_Renderer* renderer, const SDL_Rect& cameraRect) const
{
	for (Entity systemEntity : GetSystemEntities())
	{
		...
		const glm::vec3 lerpedColorVector = glm::lerp(glm::vec3(255, 0, 0), glm::vec3(0, 255, 0), healthNormalized);
		// Create sdl color with lerpedColorVector.
		const SDL_Color healthBarColor = { static_cast<Uint8>(lerpedColorVector.x),static_cast<Uint8>(lerpedColorVector.y),static_cast<Uint8>(lerpedColorVector.z),255 };
		...
		// Set our rect color.
		SDL_SetRenderDrawColor(renderer, static_cast<Uint8>(lerpedColorVector.x), static_cast<Uint8>(lerpedColorVector.y), static_cast<Uint8>(lerpedColorVector.z), 255);
