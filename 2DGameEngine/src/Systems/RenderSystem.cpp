...
namespace gala
{
    ...
    void RenderSystem::Update(SDL_Renderer* renderer, const SDL_Rect& cameraRect, const std::unique_ptr<AssetStore>& assetStore)
    {
        assert(renderer);
        ...
        std::ranges::sort(renderableEntities, [](const RenderableEntity& lhs, const RenderableEntity& rhs)
        {
        ...
        for (const auto& [transformComponent, spriteComponent] : renderableEntities)
        {
            ...
            SDL_RenderCopyEx(renderer, assetStore->GetTexture(spriteComponent.ImageFilePath), &srcRect, &dstRect
