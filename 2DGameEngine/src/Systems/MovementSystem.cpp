...
void MovementSystem::Update(const double deltaTime, const std::unique_ptr<Registry>& registry)
{
    ForEachSystemEntity([&](Entity systemEntity)
    {
        ...
        if (IsNotInMap(glm::vec2(20, 20)) && systemEntity.HasTag(TAG_PLAYER))
        {
            return;
        }
        ...
        if (systemEntity.HasComponent<SpriteComponent>())
        {
            const auto spriteComponent = systemEntity.GetComponent<SpriteComponent>();

            entityBoundCheck = {spriteComponent.Width, spriteComponent.Height};
        }
        if (IsNotInMap(entityBoundCheck, false))
        {
            registry->KillEntity(systemEntity);
            return;
        }
