...
LevelLoader::LevelLoader(Registry* registry, AssetStore* assetStore, SDL_Renderer* renderer, sol::state_view luaState)
	: RegistryPtr(registry), AssetStorePtr(assetStore), Renderer(renderer), Lua(std::move(luaState))
{
}
...
void LevelLoader::LoadLevel(const int level)
{
	...
	if (const sol::load_result script = Lua.load_file(levelFile); !script.valid())
	{
...
{
	const sol::table assetsTable = Lua["Level"]["assets"];
...
{
	const sol::table tilemapTable = Lua["Level"]["tilemap"];
...
{
	const sol::table entitiesTable = Lua["Level"]["entities"];
	...
	for (int i = 0; i < static_cast<int>(entitiesTable.size()) + 1; ++i)
	{
		...
		// All of the components is in this table.
		...
		for (const auto& [key, value] : componentsTable)
		{
			...
			if (componentName == "text")
			{
				...
				const SDL_Color color = componentValuesTable["color"];
